                                ...
                                <tbody>
                                    @foreach($portofolios as $port)
                                        <tr>
                                            ...
                                            <td>
                                                <div style="display: flex; gap: 8px;">
                                                    <button onclick="openEditPortofolioModal('{{ $port->id }}', '{{ addslashes($port->judul) }}', '{{ addslashes($port->link) }}', '{{ addslashes($port->caption) }}', '{{ addslashes(str_replace([chr(13), chr(10)], \' \', $port->informasi)) }}', '{{ addslashes($port->framework) }}')" style="background: rgba(245, 158, 11, 0.1); color: var(--warning); border: 1px solid rgba(245, 158, 11, 0.3); padding: 8px 14px; border-radius: 8px; cursor:pointer; font-weight:700; transition: 0.2s;"><i class="fa-solid fa-pen"></i></button>
